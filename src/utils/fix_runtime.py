...
def seed_everything_fix(seed: int) -> None:
    """固定随机种子。"""
    random.seed(seed)
    np.random.seed(seed)
...
def resolve_device_fix(device: str) -> torch.device:
...
def ensure_dir_fix(path: str | Path) -> Path:
    """确保目录存在，并返回 `Path`。"""
    target = Path(path)
...
def dump_config_snapshot_fix(config: dict[str, Any], output_dir: str | Path) -> Path:
    """将 fix 配置字典写入 YAML 快照。"""
    out_dir = ensure_dir_fix(output_dir)
    target = out_dir / "config_snapshot_fix.yaml"
    target.write_text(yaml.safe_dump(config, allow_unicode=True, sort_keys=False), encoding="utf-8")
    return target


def save_checkpoint_fix(
    path: str | Path,
...
def load_checkpoint_fix(
    ...
    """加载 fix 版 checkpoint，并按需恢复优化器与 scaler。"""
...
def generate_demo_signals_fix(batch_size: int = 1, signal_length: int = 512, seed: int = 42) -> dict[str, torch.Tensor]:
