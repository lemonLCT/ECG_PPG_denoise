...
from fix_model.conditional_model import ConditionalNoiseModel1D
from fix_model.diffusion import Diffusion1D
from losses.diffusion_losses_fix import FixDiffusionLoss
...
class DDPM(nn.Module):
    ...
    def __init__(self, base_model: nn.Module | type[nn.Module] | None, config: dict, device: str | torch.device) -> None:
        ...
        self.loss_fn = FixDiffusionLoss(config.get("loss", {}))
    ...
    def trainning(
        ...
        - `modality_mask:[2]` 或 `[B,2]`

        输出:
        - `total_loss/diffusion_loss/derivative_loss`
        """
    ...
    def generate(
        ...
        """
        输入:
        - `noisy_ecg/noisy_ppg:[B,1,T]` 或 `[B,T]`
        - `modality_mask:[2]` 或 `[B,2]`
    ...
    def forward(
        ...
        """
        - `train_gen_flag=0`：训练路径，返回 loss 字典
        - `train_gen_flag=1`：推理路径，返回去噪结果
        """
        if train_gen_flag == 0:
