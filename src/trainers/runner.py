...
class TrainEngine:
    ...
    def _resolve_stage_mask(self) -> Tensor | None:
        if self.stage == "ecg_pretrain":
            return torch.tensor([1.0, 0.0], device=self.device)
        if self.stage == "ppg_pretrain":
            return torch.tensor([0.0, 1.0], device=self.device)
        if self.stage == "joint":
            return None
        raise ValueError(f"未知训练阶段: {self.stage}")
    ...
    def train_one_epoch(self, dataloader: DataLoader, max_steps: int, logger: logging.Logger) -> Dict[str, float]:
        self.model.train()
        sums = {"total_loss": 0.0, "diffusion_loss": 0.0}
        step_count = 0
        ...
        with self._progress_bar(total_steps=total_steps, desc="Train") as progress_bar:
            for step, batch in enumerate(dataloader):
                ...
                modality_mask = self._resolve_stage_mask()
...
class ExperimentRunner:
    ...
    def run_smoke(self) -> None:
        ...
        modality_mask = torch.tensor([1.0, 1.0], device=device)
