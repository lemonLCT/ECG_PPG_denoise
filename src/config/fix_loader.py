...
DEFAULT_FIX_CONFIG_PATH = PROJECT_ROOT / "src" / "config" / "base.yaml"
...
def _validate_fix_config(payload: dict[str, Any]) -> None:
...
def load_fix_config(config_path: str | Path | None = None) -> dict[str, Any]:
    """读取 fix 版 YAML 配置，并返回经过基础校验的嵌套字典。"""
    target = Path(config_path) if config_path is not None else DEFAULT_FIX_CONFIG_PATH
