...
def list_available_models(config_path: Path | None = None) -> list[str]:
    ...
    variants = models_section.get("variants", {})
...
def list_available_train_stages(config_path: Path | None = None) -> list[str]:
    ...
    stages = training_section.get("stages", {})
...
def select_model_section(payload: Dict[str, Any], model_name: str | None = None) -> Dict[str, Any]:
    models_section = payload.get("models", {})
    variants = models_section.get("variants", {})
    selected_name = model_name or models_section.get("selected")
    if not selected_name:
        raise ValueError("配置文件缺少 models.selected，且未传入 model_name")
...
def select_train_section(payload: Dict[str, Any], stage: str | None = None) -> Dict[str, Any]:
    training_section = payload.get("training", {})
    common_payload = deepcopy(training_section.get("common", {}))
    stages = training_section.get("stages", {})
    selected_stage = stage or training_section.get("selected_stage") or "ecg_pretrain"
