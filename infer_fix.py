...
from dataset import load_multimodal_arrays
from fix_model import DDPM
from utils.fix_runtime import ensure_dir_fix, generate_demo_signals_fix, resolve_device_fix
from utils.logging import build_logger


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="推理 fix 版多模态扩散模型")
    ...
    parser.add_argument("--output-path", type=str, default=None, help="结果输出路径")
...
def _build_mask(mode: str, device: torch.device) -> Optional[torch.Tensor]:
    if mode == "joint":
...
def main() -> int:
    ...
    if input_path:
        ...
        demo = generate_demo_signals_fix(batch_size=1, signal_length=int(cfg["data"]["window_length"]), seed=int(cfg["runtime"]["seed"]))
    ...
    ensure_dir_fix(output_path.parent)
    payload = {
        "modality_mask": result["modality_mask"].detach().cpu().numpy(),
        "denoised_ecg": result["denoised_ecg"].detach().cpu().numpy(),
        "denoised_ppg": result["denoised_ppg"].detach().cpu().numpy(),
    }
    if y_ecg is not None:
