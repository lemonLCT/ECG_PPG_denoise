...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="训练 fix 版多模态扩散模型")
...
def main() -> int:
    ...
    logger = build_logger("train_fix")
    device = resolve_device_fix(cfg["runtime"]["device"])
    seed_everything_fix(int(cfg["runtime"]["seed"]))
    output_dir = ensure_dir_fix(cfg["path"]["train_output_dir"])
    ckpt_dir = ensure_dir_fix(output_dir / "checkpoints")
    dump_config_snapshot_fix(cfg, output_dir)
    ...
    global_step = 0
    if args.resume:
        payload = load_checkpoint_fix(args.resume, model=model, optimizer=optimizer, scaler=scaler, map_location=device)
        start_epoch = int(payload.get("epoch", 0)) + 1
        global_step = int(payload.get("global_step", 0))
        logger.info("已恢复训练: epoch=%d global_step=%d", start_epoch, global_step)

    engine = TrainEngineFix(
        model=model,
        optimizer=optimizer,
        scaler=scaler,
        device=device,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        latest_path = ckpt_dir / "latest_fix.pt"
        save_checkpoint_fix(
            path=latest_path,
            model=model,
            optimizer=optimizer,
        ...
        if (epoch + 1) % int(cfg["train"]["save_every_epochs"]) == 0:
            epoch_path = ckpt_dir / f"epoch_fix_{epoch + 1:04d}.pt"
            save_checkpoint_fix(
                path=epoch_path,
                model=model,
                optimizer=optimizer,
        ...
        if val_metrics["total_loss"] < best_val:
            ...
            best_path = ckpt_dir / "best_fix.pt"
            save_checkpoint_fix(
                path=best_path,
                model=model,
                optimizer=optimizer,
